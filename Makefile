...
#DEV
PG_DB_URL=postgresql://postgres:'1234'@localhost:5432/service-hub-test?sslmode=disable
...
gen.pg.migration: check-migrations-cmd
	migrate create -ext sql -dir $(PG_MIGRATIONS_FOLDER)$(folder) --seq $(name)
...
pg.migrate.up.schema:
	go run main.go -schema=$(schema) -folder=$(folder)
...
new-directory:
	mkdir ./src/$(name)
	mkdir ./src/$(name)/application
	mkdir ./src/$(name)/service